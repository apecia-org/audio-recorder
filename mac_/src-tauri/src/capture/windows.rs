...
use windows::core::{implement, Interface, GUID, HRESULT, PCWSTR};
...
unsafe fn run_capture(
    ...
    let mut op: Option<IActivateAudioInterfaceAsyncOperation> = None;
    ...
    let op = op.ok_or_else(|| "no async op".to_string())?;
...
unsafe fn set_propvariant_blob(
    ...
    pv.Anonymous.Anonymous.Anonymous.blob.cbSize = size;
    pv.Anonymous.Anonymous.Anonymous.blob.pBlobData = ptr as *mut u8;
}
